...
1. Read `task_scope.md`, `task_output.md`, `execution_plan.md` (if present), linked evidence, `missing_inputs.md`, and the current task conversation (employee/user messages only).
2. Read `lbai_system/prompts/finish_review_enrichment_prompt_v1.md` and produce finish review enrichment JSON per `lbai_system/schemas/finish_review_enrichment_schema_v1.json`, including `employee_conversation_turns`.
