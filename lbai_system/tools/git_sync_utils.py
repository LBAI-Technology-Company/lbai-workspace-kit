...
try:
    from lbai.git_sync import pull_remote_before_push, push_with_remote_sync
except ImportError:
    import os
    import subprocess
    ...
    def run_git(root: Path, args: list[str], env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
        ...
        if env:
            merged.update(env)
        return subprocess.run(['git', *args], cwd=root, env=merged, text=True, capture_output=True)
    ...
    def pull_remote_before_push(
        ...
        if fetch.returncode != 0:
            return False, f'git fetch failed: {(fetch.stdout + fetch.stderr).strip()}'
    ...
    def push_with_remote_sync(
        ...
        *,
        ...
        if push.returncode != 0:
            return False, 'PUSH_FAILED', (push.stdout + push.stderr).strip()
        return True, pull_detail, 'git push completed'

__all__ = ['pull_remote_before_push', 'push_with_remote_sync']
