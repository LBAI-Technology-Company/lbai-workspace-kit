...
| `/lbai-finish-task` / `lbai_finish_task` | LBAI Finish Task | 交付（按需）+ 审稿 + 同步 |
| `/lbai-execute-task` / `lbai_execute_task` | LBAI Execute Task | **高级/调试：** 只重生成交付物，不同步 |
...
/lbai-finish-task 结束（按需自动生成交付物 → 审查 → hygiene → push GitHub）
```

高级/调试：`/lbai-execute-task` 只重生成交付物（`execution_plan.md` + `task_output.md`），不同步。
