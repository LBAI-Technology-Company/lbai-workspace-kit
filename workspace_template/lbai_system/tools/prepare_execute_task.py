...
def build_plan_template(root: Path, task_dir: Path) -> str:
    ...
    return (
        ...
        '## facts_from_sources\n'
        '- TODO: 从 task_scope、task_slot、linked evidence brief 中提取可验证事实。\n\n'
        '## assumptions\n'
...
def main() -> int:
    ...
    if missing:
        ...
        for item in missing:
            print(f'- {item}')
        print(f'next_step: 请先粘贴缺失资料，使用 /lbai-add-evidence {task_dir.relative_to(root)} 补齐。')
        return 1
    ...
    print('next_step: 在 Cursor 或 Codex 桌面 App 中读取 execution_plan.md，并按计划写 task_output.md；完成后运行 /lbai-finish-task。')
    return 0
