...
This is the **normal employee end command**. It auto-runs delivery (the same work as `/lbai-execute-task`) when `task_output.md` is not ready.

Flow:

1. Resolve task folder if omitted (`resolve_current_task.py finish`).
2. Archive chat clarifications with `archive_input.py --resolves "<exact missing input>"` when the employee provided decisions or context in this conversation.
3. Run `python3 lbai_system/tools/check_task_delivery.py <task_folder>`.
4. **Auto-execute when needed** (`auto_execute_needed: true`):
...
If AI enrichment is unavailable for finish review, return blocked finish. Do not call `finish_task.py` without `--enrichment`.
