# WORKFLOW_BASE_RESPONSIBILITY_GAP_REGISTER_v1

| gap_id | priority | gap_name | gap_class | current_evidence | missing_artifact | owner | next_chain |
|---|---|---|---|---|---|---|---|
| GAP-01 | P1 | Agent identity lock | Scope | README and commands describe a workspace kit | Identity source and scope lock | Workflow owner | Scope lock |
...
| GAP-03 | P0 | Workspace Bootstrap Agent | Bootstrap | Manual README setup plus command checker | Bootstrap check/fill contract and tool | Workflow owner | Bootstrap v1 |
...
| GAP-07 | P0 | Structured task ledger | Ledger | Task and global ledgers are markdown-light | Canonical ledger schema | Workflow owner | Task Ledger v1 |
| GAP-08 | P1 | External evidence intake boundary | Evidence | `/lbai-add-evidence` captures evidence under `role_workspace/knowledge/evidence/` and links task gaps | Stronger admissibility review and role-delta conversion rules | Workflow owner | Evidence intake v0.6 hardening |
...
| GAP-10 | P0 | GitHub Sync / Repo Hygiene Agent | Artifact ledger | Hygiene check exists; push is manual | Auto sync contract and status fields | Workflow owner | GitHub Sync v1 |
