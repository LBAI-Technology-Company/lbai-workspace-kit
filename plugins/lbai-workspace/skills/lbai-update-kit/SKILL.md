...
1. Run `lbai doctor --json --plugin-version 1.4.22 --min-workspace-version 1.4.1`, but allow `workspace_update_required` to proceed. Doctor resolves the registered active workspace even when Codex is opened on another project.
...
5. Run `lbai doctor --json --plugin-version 1.4.22 --min-workspace-version 1.4.1` again.
